(() => {
  ...
  if (detail) {
    ...
    const summaryCards = `<section class="briefing-summary" aria-label="Meeting highlights">
      ...
      <article id="action-items" class="briefing-summary__card"><span class="briefing-summary__icon" aria-hidden="true">✓</span><div><h2>Action items</h2><ul>${(m.actionItems || []).map(item => `<li>${esc(item)}</li>`).join('')}</ul></div><a href="#cif-arbiter">View all action items <span aria-hidden="true">→</span></a></article>
    ...
    const navLinks = navigation.map(({id,label,index}) => `<li><a href="#${id}"><b>${String(index + 1).padStart(2, '0')}</b><span>${esc(label)}</span></a></li>`).join('');
