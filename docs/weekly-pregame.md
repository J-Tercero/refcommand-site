...
2. Change its snapshot, referee message, game notes, crew, focuses, optional situation, five quiz questions, acknowledgment label, and recap items. Keep each quiz question ID unique inside the record. The acknowledgment position/name dropdown is generated from the `crew` array.
...
Quiz answers, the selected crew member and receipt acknowledgment, acknowledgment timestamp, question draft, and recap checks use `localStorage` keys shaped as `refcommand:pregame:<slug>:<area>`. State is device/browser-specific, is not authenticated, and is not reported to a referee. The pure helpers in `assets/js/pregame-core.js` keep quiz scoring and acknowledgment eligibility separate from rendering.
