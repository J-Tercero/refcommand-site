...
test('pregame library and webmaster link to the active game and preserved template', () => {
  const webmaster = fs.readFileSync(path.join(root, 'pages/webmaster.html'), 'utf8');
  const library = fs.readFileSync(path.join(root, 'pages/pregame.html'), 'utf8');
  const template = pregames.find(({ slug }) => slug === 'central-at-clovis-west-2026-09-18');
  assert.match(webmaster, new RegExp(`href="../pregame/${game.slug}/"`));
  assert.match(webmaster, new RegExp(`href="../pregame/${template.slug}/"`));
  assert.match(library, new RegExp(`href="../pregame/${game.slug}/"`));
  assert.match(library, new RegExp(`href="../pregame/${template.slug}/"`));
  assert.ok(template.quiz.length === 5 && template.crew.length === 7);
  assert.ok(fs.existsSync(path.join(root, 'pregame', template.slug, 'index.html')));
});
